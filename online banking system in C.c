...
int main(){
	...
	if(option == 1){
			...
			if(!strcmp(password,user.password)){
				...
				switch(choice){
					...
					case 2:
						...
						if(fwrite !=0) 
						printf("\n\nYou have Succesfully Depostied Rs.%d",amount);
						fclose(fp);
						break;
						...
						else 
						{
							...
							else 
							{
								...
								if(fwrite != 0)
								{
									...
									printf("Your transfer is completed. You have trasnfered Rs.%d to %s",amount,usr.phone);
									fclose(fptr);
									user.balance -= amount;
									strcpy(filename,user.phone);
									fp = fopen(strcat(filename,".dat"),"w");
		...
		printf("\n\n***Thank you for banking with SUMIT.bank***\n\n");
	}
